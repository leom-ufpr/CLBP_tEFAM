function [featureMatrix] = computeBPDescriptorsMSB(metaData, srcPath, imgPath, method, rmax, radius, neighbors)
...
addpath([srcPath 'src/BC']);
  ...
  switch method
    case 'lbp'
      featureMatrix = LBPDescriptorsMSB(imgPath,srcPath,dataClasses,'lbp',radius,neighbors);
    case 'lbpSMC'
      featureMatrix = LBPDescriptorsMSB(imgPath,srcPath,dataClasses,'clbp_s/m/c',radius,neighbors);
    otherwise
      for i = 1:noFiles
        tic;
        fileName = metaData(i).name;
        img = double(imread([imgPath fileName]));
        switch method
          case 'mink'
